...
public class PaymentCommandKafkaListener {
    ...
    public void handleDlt(ConsumerRecord<String, ?> consumerRecord) {
        ...
        paymentInboxJpaRepository.findByTopicAndPartitionAndOffset(consumerRecord.topic(), consumerRecord.partition(), consumerRecord.offset())
                .ifPresent(PaymentInboxEntity::markDeadLettered);
    }
}
